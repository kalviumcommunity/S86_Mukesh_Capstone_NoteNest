...
const GoogleOAuthButton = ({ text = "Continue with Google" }) => {
  const handleGoogleAuth = () => {
    // Fallback to window.location.origin if VITE_BACKEND_URL is not provided
    const backend = DEPLOY_URL || window.location.origin;
    ...
    window.location.assign(`${normalized}/api/auth/google`);
  };
